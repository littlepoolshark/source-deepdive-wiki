...
关于 Feature Flags 如何在 Release Channel 内控制单个功能的详细信息，请参阅 [Feature Flags System](/2-feature-flags-system)。关于处理这些 Channel 的构建流水线的详细信息，请参阅 [Build Pipeline and Module Forking](/3.1-build-pipeline-and-module-forking)。
...
核心逻辑出现在 [scripts/rollup/bundles.js#L3-L8](https://github.com/facebook/react/blob/main/scripts/rollup/bundles.js#L3-L8) 和 [scripts/rollup/build.js#L31-L38](https://github.com/facebook/react/blob/main/scripts/rollup/build.js#L31-L38)：
...
来源：[scripts/rollup/bundles.js#L3-L8](https://github.com/facebook/react/blob/main/scripts/rollup/bundles.js#L3-L8)、[scripts/rollup/build.js#L31-L38](https://github.com/facebook/react/blob/main/scripts/rollup/build.js#L31-L38)、[scripts/rollup/build.js#L432-L442](https://github.com/facebook/react/blob/main/scripts/rollup/build.js#L432-L442)
...
构建系统在 [scripts/rollup/bundles.js#L10-L31](https://github.com/facebook/react/blob/main/scripts/rollup/bundles.js#L10-L31) 中定义 Bundle 类型：
...
分类逻辑在 [scripts/rollup/build.js#L253-L310](https://github.com/facebook/react/blob/main/scripts/rollup/build.js#L253-L310)：
...
来源：[scripts/rollup/bundles.js#L10-L54](https://github.com/facebook/react/blob/main/scripts/rollup/bundles.js#L10-L54)、[scripts/rollup/build.js#L225-L310](https://github.com/facebook/react/blob/main/scripts/rollup/build.js#L225-L310)、[scripts/rollup/build.js#L312-L338](https://github.com/facebook/react/blob/main/scripts/rollup/build.js#L312-L338)
...
实现在 [scripts/rollup/build.js#L585-L633](https://github.com/facebook/react/blob/main/scripts/rollup/build.js#L585-L633)：
...
来源：[scripts/rollup/build.js#L585-L633](https://github.com/facebook/react/blob/main/scripts/rollup/build.js#L585-L633)、[scripts/jest/setupHostConfigs.js#L7-L99](https://github.com/facebook/react/blob/main/scripts/jest/setupHostConfigs.js#L7-L99)

---

## 与 Feature Flags 的交互

Release Channel 直接影响加载哪些 Feature Flag 配置。Fork 解析系统（在 [Feature Flags System](/2-feature-flags-system) 中记录）根据 Bundle 类型和 Release Channel 选择 Flag 文件。

### Feature Flag Fork 选择

[scripts/rollup/forks.js#L134-L191](https://github.com/facebook/react/blob/main/scripts/rollup/forks.js#L134-L191) 中的 Feature Flag Fork 逻辑展示了这种关系：
...
[packages/shared/ReactFeatureFlags.js#L75-L100](https://github.com/facebook/react/blob/main/packages/shared/ReactFeatureFlags.js#L75-L100) 中的示例：
...
来源：[scripts/rollup/forks.js#L134-L191](https://github.com/facebook/react/blob/main/scripts/rollup/forks.js#L134-L191)、[packages/shared/ReactFeatureFlags.js#L75-L100](https://github.com/facebook/react/blob/main/packages/shared/ReactFeatureFlags.js#L75-L100)、[packages/shared/forks/ReactFeatureFlags.www.js#L1-L119](https://github.com/facebook/react/blob/main/packages/shared/forks/ReactFeatureFlags.www.js#L1-L119)
...
[scripts/rollup/packaging.js#L48-L115](https://github.com/facebook/react/blob/main/scripts/rollup/packaging.js#L48-L115) 中的 `getBundleOutputPath()` 函数决定每个 Bundle 的写入位置：
...
来源：[scripts/rollup/packaging.js#L48-L115](https://github.com/facebook/react/blob/main/scripts/rollup/packaging.js#L48-L115)、[scripts/rollup/packaging.js#L117-L137](https://github.com/facebook/react/blob/main/scripts/rollup/packaging.js#L117-L137)
...
[scripts/rollup/packaging.js#L253-L284](https://github.com/facebook/react/blob/main/scripts/rollup/packaging.js#L253-L284) 中的 `prepareNpmPackages()` 函数执行以下步骤：
...
[scripts/rollup/packaging.js#L171-L251](https://github.com/facebook/react/blob/main/scripts/rollup/packaging.js#L171-L251) 中的 `filterOutEntrypoints()` 函数从 package.json 中移除未为当前 Release Channel 构建的入口点：
...
该函数使用在 [scripts/rollup/packaging.js#L158-L169](https://github.com/facebook/react/blob/main/scripts/rollup/packaging.js#L158-L169) 中构建的 `entryPointsToHasBundle` 映射：
...
来自 [scripts/rollup/packaging.js#L197-L205](https://github.com/facebook/react/blob/main/scripts/rollup/packaging.js#L197-L205)。

### 服务器入口点分发

React 提供多个服务器渲染入口点，根据 `process.env.NODE_ENV` 进行条件加载。这些在 npm 包装文件中演示：

**Node.js Server** - [packages/react-dom/npm/server.node.js#L1-L19](https://github.com/facebook/react/blob/main/packages/react-dom/npm/server.node.js#L1-L19)：
```javascript
var l, s;
...
**Browser Server** - [packages/react-dom/npm/server.browser.js#L1-L17](https://github.com/facebook/react/blob/main/packages/react-dom/npm/server.browser.js#L1-L17)

**Bun Server** - [packages/react-dom/npm/server.bun.js#L1-L20](https://github.com/facebook/react/blob/main/packages/react-dom/npm/server.bun.js#L1-L20)

**Edge Runtime** - [packages/react-dom/npm/server.edge.js#L1-L18](https://github.com/facebook/react/blob/main/packages/react-dom/npm/server.edge.js#L1-L18)

每个运行时根据能力提供略有不同的 API 表面。例如，`renderToPipeableStream` 仅在 Node.js 和 Bun 中可用，在浏览器中不可用。

来源：[scripts/rollup/packaging.js#L158-L273](https://github.com/facebook/react/blob/main/scripts/rollup/packaging.js#L158-L273)、[packages/react-dom/npm/server.node.js#L1-L19](https://github.com/facebook/react/blob/main/packages/react-dom/npm/server.node.js#L1-L19)、[packages/react-dom/npm/server.browser.js#L1-L17](https://github.com/facebook/react/blob/main/packages/react-dom/npm/server.browser.js#L1-L17)、[packages/react-dom/npm/server.bun.js#L1-L20](https://github.com/facebook/react/blob/main/packages/react-dom/npm/server.bun.js#L1-L20)

---

## 测试中的 Release Channel 使用

测试基础设施通过环境设置遵循 Release Channel。[scripts/jest/setupHostConfigs.js#L7-L99](https://github.com/facebook/react/blob/main/scripts/jest/setupHostConfigs.js#L7-L99) 中的 `setupHostConfigs.js` 文件使用相同的 `resolveEntryFork()` 逻辑来确保测试加载正确的入口点。
...
来自 [scripts/jest/setupHostConfigs.js#L101-L115](https://github.com/facebook/react/blob/main/scripts/jest/setupHostConfigs.js#L101-L115)。

来源：[scripts/jest/setupHostConfigs.js#L7-L115](https://github.com/facebook/react/blob/main/scripts/jest/setupHostConfigs.js#L7-L115)
